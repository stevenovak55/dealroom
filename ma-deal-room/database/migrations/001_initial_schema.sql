...
-- ============================================================================
-- Description: Creates all 8 core tables for MA Deal Room plugin
-- Created: 2025-10-30
-- Idempotent: Yes (CREATE TABLE IF NOT EXISTS)
-- Rollback: See rollback_001.sql
-- ============================================================================

-- Migration tracking table (creates itself if needed)
CREATE TABLE IF NOT EXISTS `{prefix}ma_deal_migrations` (
...
SET @migration_exists = (SELECT COUNT(*) FROM `{prefix}ma_deal_migrations` WHERE `migration_number` = '001');

-- Only proceed if migration not yet applied
SET @skip_migration = IF(@migration_exists > 0, 1, 0);

-- ============================================================================
-- Execute migration (wrapped in conditional)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS `{prefix}ma_deal_transactions` (
  `id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `mls_number` VARCHAR(50) DEFAULT NULL COMMENT 'MLS listing number if imported from MLS',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  UNIQUE KEY `uk_account_mls` (`account_id`, `mls_number`),
...
-- Table 3: Parties
CREATE TABLE IF NOT EXISTS `{prefix}ma_deal_parties` (
  `id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- Table 4: Templates
CREATE TABLE IF NOT EXISTS `{prefix}ma_deal_templates` (
  `id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- Table 5: Tasks
CREATE TABLE IF NOT EXISTS `{prefix}ma_deal_tasks` (
  `id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
  `transaction_id` BIGINT(20) UNSIGNED NOT NULL,
  ...
  CONSTRAINT `fk_task_template` FOREIGN KEY (`template_id`)
    REFERENCES `{prefix}ma_deal_templates` (`id`) ON DELETE SET NULL,
  CONSTRAINT `fk_task_party` FOREIGN KEY (`assigned_party_id`)
    REFERENCES `{prefix}ma_deal_parties` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Instantiated tasks with status, due dates, and dependencies';

-- Table 6: Events
CREATE TABLE IF NOT EXISTS `{prefix}ma_deal_events` (
  `id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
  `account_id` BIGINT(20) UNSIGNED NOT NULL,
...
-- Table 7: Reminders
CREATE TABLE IF NOT EXISTS `{prefix}ma_deal_reminders` (
  `id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_reminder_task` FOREIGN KEY (`task_id`)
    REFERENCES `{prefix}ma_deal_tasks` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_reminder_transaction` FOREIGN KEY (`transaction_id`)
    REFERENCES `{prefix}ma_deal_transactions` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Reminder queue for email/SMS notifications';

-- Table 8: Vendor Requests
CREATE TABLE IF NOT EXISTS `{prefix}ma_deal_vendor_requests` (
  `id` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT,
  `task_id` BIGINT(20) UNSIGNED NOT NULL,
...
INSERT INTO `{prefix}ma_deal_migrations` (`migration_number`, `migration_name`, `applied_at`, `rollback_available`)
VALUES ('001', 'Initial Schema - 8 Core Tables', NOW(), TRUE)
ON DUPLICATE KEY UPDATE `migration_name` = `migration_name`; -- No-op if already exists
