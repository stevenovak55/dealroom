...
export function calculateMilestonesFromOfferAcceptance(offerAcceptedDate) {
 ...
 */
export function calculatePSDate(offerAcceptedDate) {
    const baseDate = typeof offerAcceptedDate === 'string'
        ? parseDate(offerAcceptedDate)
        : offerAcceptedDate;
    if (!baseDate)
 ...
 */
export function calculateLoanCommitmentDate(psDate) {
 ...
 */
export function calculateClosingDate(loanCommitmentDate) {
 ...
 */
export function calculateClosingDateFromPS(psDate) {
    const baseDate = typeof psDate === 'string'
        ? parseDate(psDate)
        : psDate;
    if (!baseDate)
 ...
 */
export function isDueSoon(date, daysThreshold = 7) {
    const d = typeof date === 'string' ? parseDate(date) : date;
    if (!d)
        return false;
    const today = new Date();
